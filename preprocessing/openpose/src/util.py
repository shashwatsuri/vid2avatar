...
def draw_handpose(canvas, all_hand_peaks, show_number=False):
    ...
    canvas = np.fromstring(bg.tostring_rgb(), dtype='uint8').reshape(int(1024), int(1024), 3)
    return canvas
...
def handDetect(candidates, subset, oriImg):
    ...
    for person_id in range(subset.shape[0]):
