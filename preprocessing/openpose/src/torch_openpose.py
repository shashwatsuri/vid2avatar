...
from openpose.src.model import bodypose_model,bodypose_25_model, handpose_model

model_coco = './openpose/model/body_coco.pth'
model_body25 = './openpose/model/body_25.pth'

class body_25(object):
    ...
    def __call__(self, oriImg):
        ...
        multiplier_x = [x * boxsize_x / oriImg.shape[0] for x in scale_search]
        multiplier_y = [x * boxsize_y / oriImg.shape[1] for x in scale_search]
        ...
        for m in range(len(multiplier_x)):
            # scale = multiplier[m]
            # imageToTest = cv2.resize(oriImg, (0, 0), fx=scale, fy=scale, interpolation=cv2.INTER_CUBIC)

            scale_x = multiplier_x[m]
            scale_y = multiplier_y[m]
            imageToTest = cv2.resize(oriImg, (0, 0), fx=scale_x, fy=scale_y, interpolation=cv2.INTER_CUBIC)
            ...
            heatmap_avg += heatmap_avg + heatmap / len(multiplier_x)
            paf_avg += + paf / len(multiplier_x)
